...
void update_tree (int node, int r,int e,int ara[],int pause);
int calculate_min (int node, int r, int e, int a , int b);

//I ara[100003];    //caution always take ara[1] as the first node rather than ara[0] to avoid falls multiple vale ,eg- 2*0=0 but literally it should be 2*1=2 ;)
I tree[300009]={0};
I mn;
int main()
{
    ...
    while (t--)
    {
        ...
        scanf("%d%d",&n,&q);
        for (i=1;i<=n;i++)  //See top for knowing reason of taking i=1 is stater.
        {
                scanf("%d",&ara[i]);
                update_tree(1,1,n,ara,i);
        }
...
void update_tree (int node, int r,int e,int ara[],int pause)
{
    ...
    if (r==e)
    {
        //cout<<"node "<<node<<" update "<<ara[r]<<endl;
        tree[node]=ara[r];
        return;
    }
    int left=node<<1;
    ...
    int mid=(r+e)>>1;
    if (pause<=mid)
        update_tree(left,r,mid,ara,pause);
    else
        update_tree(right,mid+1,e,ara,pause);
...
int calculate_min (int node, int r, int e, int a , int b)
{
    ...
    if (a>mid)
        {
             return calculate_min(right,mid+1,e,a,b);
        }
    else if (b<=mid)
    {
        calculate_min(left,r,mid,a,b);
    }
    else
        return  min(calculate_min(left,r,mid,a,b),calculate_min(right,mid+1,e,a,b));
}
